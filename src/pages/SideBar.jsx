...
import AddChannelDialog from './channel/AddAChannelBg'
...
export default function Sidebar() {
    ...
    return (
        <div>
            {addServerFlag && <AddChannelDialog id={bgElementId} onClickCloseAddAServer={onClickCloseAddAServer} setAddServerFlag={setAddServerFlag} />}
            ...
            {data?.data?.map((channel) => {
                ...
                return (
                    <div key={channel.channelId} >
                        <Link href={linkTo} >
                            <div className={`hover:cursor-pointer sidebar-icon group m-1 ${isCurrActive(linkTo) ? 'sidebar-active' : ''}`}>
...
function SidebarIcon({ icon, text = 'tooltip 💡', linkTo = '/'}) {
    ...
    return (
        <div >
            <Link href={linkTo} >
                <div className={`sidebar-icon group ${isCurrActive(linkTo) ? 'sidebar-active' : ''}`}>
                    {icon}
                    {/* Styling based on parent state (group-{modifier}) */}
