...
function ChannelInfoPanel({ channelId }) {
    ...
    const closeOpenMenus = (e) => {
        if (menuElement.current && menuActive && !menuElement.current.contains(e.target)) {
            setMenuActive(false)
        }
    }
    ...
    useEffect(() => {
        ...
        return () => {
            document.removeEventListener('mousedown', closeMenuEvent)
        }
    }, [])
    ...
    return (
        <div className='relative'>
            ...
            {menuActive && <ChannelMenu menuElement={menuElement} channelId={channelId} />}
        </div>

    )
}

function ChannelMenu({ channelId, menuElement }) {
    ...
    return (
        <div ref={menuElement} className='absolute flex w-full px-7'>
            <div className='flex flex-col bg-[rgb(23,24,26)] rounded-md w-full text-gray-200 p-3'>
                <span className='hover:cursor-pointer p-1 hover:bg-[#3b3c3f] rounded-lg font-bold' onClick={onClickInvitePeople}>Invite People</span>
                <span className='hover:cursor-pointer p-1 hover:bg-[#3b3c3f] rounded-lg font-bold'>Channel Settings</span>
            </div>
